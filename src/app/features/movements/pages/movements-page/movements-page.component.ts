...
export class MovementsPageComponent implements OnInit, OnDestroy {
  ...
  deleteMovement(movement: MovementResponse): void {
    if (confirm(`¿Está seguro de que desea eliminar este movimiento?`)) {
      this.loading = true;
      this.movementService.deleteMovement(movement.id || movement.transactionId!)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.notificationService.showSuccess('Éxito', 'Movimiento eliminado correctamente');
            this.loadMovements();
          },
          error: (error) => {
            this.notificationService.showError('Error', 'No se pudo eliminar el movimiento');
            this.loading = false;
          }
        });
    }
  ...
  private updateAccountOptions(): void {
    this.accountOptions = this.accounts.map(account => {
      ...
      let balance = account.currentBalance ?? account.initialBalance ?? 0;
      
      // Si ya tenemos movimientos cargados, usar el último balance de esta cuenta
      if (this.movements.length > 0) {
        ...
        if (lastMovement && lastMovement.balance) {
          balance = lastMovement.balance;
          // Actualizar también el currentBalance del objeto account
